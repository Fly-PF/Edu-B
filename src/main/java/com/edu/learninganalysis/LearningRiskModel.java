...
public class LearningRiskModel {
    ...
    public LearningRiskResult assess(LearningRiskInput input) {
        ...
        double practiceGap = input.practiceAccuracy() == null ? 0.0 : clamp((75.0 - input.practiceAccuracy()) / 75.0);
        ...
        if (input.practiceAccuracy() != null) {
            factors.add(new LearningRiskFactor("practiceMastery", "练习掌握", (int) Math.round(practiceGap * 100),
                    "已批改练习得分率 " + input.practiceAccuracy() + "%"));
        }
        ...
        return new LearningRiskResult(score, level, estimatedDays, factors, recommendation, "Logistic regression v1.1-practice-evidence");
    }
    ...
    private String buildRecommendation(LearningRiskInput input, LearningRiskFactor topFactor) {
        return switch (topFactor.code()) {
            ...
            case "practiceMastery" -> "先回看低分题目，再完成一道同类练习确认掌握。";
